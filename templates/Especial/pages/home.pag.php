...
<div id="content">
    ...
    <div class="s-header"><span>Top's do Servidor</span></div>
    	<div id="outer-div">
			...
			<?php
			global $CTM_MSSQL;
			$query = $CTM_MSSQL->query("SELECT TOP 5 Name,".Column_Reset.",CTM_Image FROM ".MuGen_DB.".dbo.Character WHERE CtlCode < 8 ORDER BY ".Column_Reset." DESC");
			$cont = NULL;
			...
			<?php
			global $CTM_MSSQL;
			$query = $CTM_MSSQL->query("SELECT TOP 5 Name,".Column_MReset.",CTM_Image FROM ".MuGen_DB.".dbo.Character WHERE CtlCode < 8 ORDER BY ".Column_MReset." DESC");
			$cont = NULL;
			while($topresets = mssql_fetch_array($query)){
			$cont++;
				if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
			...
			<?php
			global $CTM_MSSQL;
			$query = $CTM_MSSQL->query("SELECT TOP 5 Name,".Column_ResetDay.",CTM_Image FROM ".MuGen_DB.".dbo.Character WHERE CtlCode < 8 ORDER BY ".Column_ResetDay." DESC");
			$cont = NULL;
			while($topresets = mssql_fetch_array($query)){
			$cont++;
				if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
			...
			<?php
			global $CTM_MSSQL;
			$query = $CTM_MSSQL->query("SELECT TOP 5 Name,".Column_ResetWeek.",CTM_Image FROM ".MuGen_DB.".dbo.Character WHERE CtlCode < 8 ORDER BY ".Column_ResetWeek." DESC");
			$cont = NULL;
			while($topresets = mssql_fetch_array($query)){
			$cont++;
				if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
